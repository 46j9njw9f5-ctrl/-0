...
export default function MethodologyPage() {
  useDocumentMeta({
    ...
    description:
      '将来性・生産性・働きやすさスコアの構成要素、ブラック度の算出条件、欠損データの補正方法、データソースを説明します。スコアは将来を保証しない独自の参考指標です。',
    path: '/methodology',
    ogType: 'article',
  })
  ...
  return (
    ...
    >
      ...
      <section className="page__section">
        <h2 className="page__h2">🛡 ブラック度（安全度）の算出条件</h2>
        <p>
          離職率・残業代支給率・労基署の是正勧告など、<b>公的または明示されたデータがある場合にのみ</b>
          算出します。実在企業に推測値を当てはめてブラック度を付けることはしません。データが無い企業では
          「安全度」は表示されず、働きやすさなど取得できた指標のみを示します。
        </p>
      </section>
      ...
      <section className="page__section">
        ...
        <p>
          ...
          相性診断（マッチ度）では、選択した軸の充足率が低い場合に中立値（50）へ縮約し、半数未満しか
