...
describe('企業詳細のURL・ルーティング', () => {
  ...
  it('privacy ページが表示される', async () => {
    renderApp('/privacy')
    expect(
      await screen.findByRole('heading', { name: 'プライバシーポリシー' }, { timeout: 8000 }),
    ).toBeInTheDocument()
  }, 20000)

  it('contact ページが表示される', async () => {
    renderApp('/contact')
    expect(
      await screen.findByRole('heading', { name: '運営者情報・お問い合わせ' }, { timeout: 8000 }),
    ).toBeInTheDocument()
  }, 20000)
})
