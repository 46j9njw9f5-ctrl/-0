...
import { dataConfidence, type DataConfidence } from '../engine/confidence'
...
export function CompanyDetail({
  ...
  const confidence = dataConfidence({ company, growth, productivity, stock, evaluation, workability, scores })
      ...
      >
        ...
        {tab === 'work' && workability && (
          <div className="tab-body" role="tabpanel" id="cd-panel-work" aria-labelledby="cd-tab-work" tabIndex={0}>
            ...
            {confidence.workabilityThin && (
              <div className="notice">
                🔎 この働きやすさは公開されている<b>1項目のみ</b>で算出した<b>参考値</b>です。断定は避け、
                詳細は各社の公表情報もご確認ください。
              </div>
            )}
            <FactorBars
...
function ConfidenceBar({ confidence }: { confidence: DataConfidence }) {
  const cls =
    confidence.level === 'high' ? 'lv-excellent' : confidence.level === 'mid' ? 'lv-caution' : 'lv-danger'
  return (
    <div className="confidence" role="note" aria-label="データ充足度">
      <div className="confidence__row">
        <span className={`badge ${cls}`}>
          <span className="badge__dot" aria-hidden="true" />
          データ充足度：{confidence.label}
        </span>
        <span className="confidence__count">
          {confidence.filled}/{confidence.total} 軸が実データ
          {confidence.workabilityFactors > 0 ? `・働きやすさ ${confidence.workabilityFactors}項目` : ''}
        </span>
      </div>
      <p className="confidence__note">
        {confidence.note}
        {confidence.isReference && <b>このスコアは参考値です。</b>}
        {confidence.workabilityThin && '（働きやすさは1項目のみのため、断定を避けています）'}
      </p>
    </div>
  )
}
