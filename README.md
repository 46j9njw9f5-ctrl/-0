...
- **モバイル最適化**：320〜768px で横スクロールなし。データセット切替・ビュー切替を全幅化、
  グリッドは `minmax(min(100%, …), 1fr)` で狭幅でも収まる。ヘッダーはモバイルで非固定。
- **iPhone向けボトムシート**：詳細・比較はモバイルで全画面シート化。ドラッグハンドル、
  ヘッダー＋タブの固定、`env(safe-area-inset-*)` によるノッチ/ホームバー対応、スライドアップ表示。
- **モバイル上部の圧縮**：業種・絞り込み・診断のチップを1行の横スワイプに（右端フェードで続きを示唆）、
  説明文を隠し subhead を2行に。最初の企業までの距離を約1300px→700px に短縮。
- **下部固定タブバー**：モバイルでは「企業を見る／データ分析」を画面下部の固定ナビに（半透明ブラー・
  セーフエリア対応）。比較バーはその上に重ねて共存。
